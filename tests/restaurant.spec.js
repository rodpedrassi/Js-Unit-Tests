...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const fetchResultado = objetoRetornado.fetchMenu();
    // objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}
    // ```
    expect(Object.keys(fetchResultado).length).toBe(2);
    expect(Object.keys(fetchResultado)).toContain('food');
    expect(Object.keys(fetchResultado)).toContain('drink');
    ...
    // ```
    expect(menu).toMatchObject(fetchResultado);
